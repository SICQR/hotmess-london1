  ...
  try {
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        if (order.status === "created") {
          await insertNotif(
            ...
            "order_paid",
            "Order confirmed",
            "Payment received. You can message support/seller in-app.",
            `/messmarket/orders/${orderId}`,
          );
          if (sellerOwner) {
            await insertNotif(
              sellerOwner,
              "order_paid",
              "New order paid",
              "A buyer has paid. Accept + ship within your SLA.",
              `/seller/orders`,
            );
          }
      ...
      case "account.updated": {
        ...
        await admin.from("market_sellers").update({ stripe_onboarding_complete: onboardingComplete }).eq("stripe_account_id", stripeAccountId);
