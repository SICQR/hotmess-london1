...
import trustApiApp from "./trust_api.tsx"; // NEW: Trust & safety (mute/block/report)
import attachmentsApiApp from "./attachments_api.tsx"; // NEW: Thread attachments
import analyticsApiApp from "./analytics_api.tsx"; // NEW: Analytics ingestion API
import lastfmApiApp from "./lastfm_api.tsx"; // NEW: Last.fm integration API
import shopifyApiApp from "./shopify_api.tsx"; // NEW: Shopify Storefront proxy (avoids browser CORS)
...
import { sendWelcomeEmail } from "./email_service.tsx";
...
// NOTE: Supabase clients send `apikey` and `x-client-info` headers; include them to prevent preflight failures.
app.use(
  "*",
  cors({
    origin: "*",
...
// Explicit preflight handler (fixes OPTIONS blocks)
...
// Alias health check for environments where Supabase strips the function-name prefix
app.get("/health", (c) => {
  return c.json({ 
    status: "ok", 
    timestamp: Date.now(), 
...
// Alias health check for clients expecting an /api prefix
...
// Alias health check for clients expecting /make-server-a670c824/api prefix
app.get("/make-server-a670c824/api/health", (c) => {
  return c.json({ 
    status: "ok", 
    timestamp: Date.now(), 
    version: "1.0.1",
...
app.route("/api/beacons", beaconApiApp);
...
app.route("/api/intel", intelApiApp);

// Mount Map API routes (heat/trail data)
app.route("/make-server-a670c824/api/map", mapApiApp);
app.route("/api/map", mapApiApp);

// Mount Admin Console API routes
app.route("/make-server-a670c824/api/admin", adminApiApp);
app.route("/api/admin", adminApiApp);

// Mount Night Pulse Heat Map API routes
app.route("/make-server-a670c824/api/heat", heatApiApp);
app.route("/api/heat", heatApiApp);

// Mount Trust & Safety API routes
app.route("/make-server-a670c824/api/trust", trustApiApp);
app.route("/api/trust", trustApiApp);

// Mount Thread Attachments API routes
app.route("/make-server-a670c824/api/threads/attachments", attachmentsApiApp);
app.route("/api/threads/attachments", attachmentsApiApp);
...
const handleBeaconScan = async (c: any) => {
  try {
...
app.post("/make-server-a670c824/api/beacons/:id/scan", handleBeaconScan);
app.post("/api/beacons/:id/scan", handleBeaconScan);

// POST /api/beacons/:id/action - Validate premium/GPS + award XP_ACTION
const handleBeaconAction = async (c: any) => {
  try {
    const beaconId = c.req.param('id');
    const body = await c.req.json();
...
app.post("/make-server-a670c824/api/beacons/:id/action", handleBeaconAction);
app.post("/api/beacons/:id/action", handleBeaconAction);

// POST /api/notify/subscribe - Subscribe to beacon notifications
const handleNotifySubscribe = async (c: any) => {
  try {
    const body = await c.req.json();
...
app.post("/make-server-a670c824/api/notify/subscribe", handleNotifySubscribe);
app.post("/api/notify/subscribe", handleNotifySubscribe);
