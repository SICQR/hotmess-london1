/**
 * BEACON OS DEMO - Home Page
 * Directory of all 6 demo beacons
 * Connected to demo backend on localhost:3001
 */

import { BeaconDemoNav } from '../components/BeaconDemoNav';

interface BeaconsDemoHomeProps {
  onNavigate: (route: string, params?: Record<string, string>) => void;
}

export function BeaconsDemoHome({ onNavigate }: BeaconsDemoHomeProps) {
  const demoBeacons = [
    {
      code: "DEMO_CHECKIN",
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      {/* Demo Nav Bar */}
      <BeaconDemoNav currentRoute="beaconsDemoHome" onNavigate={onNavigate} />
      ...
      <div className="max-w-4xl mx-auto px-4 py-12">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {demoBeacons.map((beacon) => (
            <button
              key={beacon.code}
              onClick={() => onNavigate('beaconScanDemo', { code: beacon.code })}
              className="block text-left border border-neutral-800 rounded-xl p-5 hover:border-neutral-700 transition bg-neutral-950/50 backdrop-blur"
            >
        ...
        <div className="mt-12 border-t border-neutral-800 pt-8">
          ...
          <div className="space-y-3">
            <button
              ...
              className="w-full text-left block border border-neutral-800 rounded-xl p-4 hover:border-neutral-700 transition bg-neutral-950/50"
            >
              <div className="flex items-center justify-between">
                <div>
