...
### Run the Beacon demo backend (optional, recommended)
The frontend proxies `/l` and `/x` demo endpoints to a local backend on **http://localhost:3001**.
...
This repo is designed to open and run without creating local env files.
Runtime config lives in **`src/lib/env.ts`** and the UI will surface “bridge unavailable” states when a third‑party integration is not configured.
