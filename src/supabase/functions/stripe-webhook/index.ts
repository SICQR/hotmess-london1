...
Deno.serve(async (req) => {
  ...
  async function markOrderPaid(orderId: string, patch: Record<string, unknown>) {
    // Some deployments may not yet have newer columns (e.g. paid_at). Try once,
    // then retry without unknown fields if PostgREST complains.
  ...
  async function awardXpBonusIdempotent(params: {
    ...
    // Newer schema: xp_ledger(user_id, reason, amount, meta)
    try {
      ...
      // Fall through to legacy if columns don't exist.
    ...
    // Legacy schema fallback: xp_ledger(user_id, kind, amount, ref_id, metadata)
  ...
  async function activateBeaconFromMetadata(metadata: Record<string, string> | null | undefined) {
    ...
    const lat = metadata?.user_lat ? Number(metadata.user_lat) : metadata?.geo_lat ? Number(metadata.geo_lat) : NaN;
    const lng = metadata?.user_lng ? Number(metadata.user_lng) : metadata?.geo_lng ? Number(metadata.geo_lng) : NaN;
    const city = (metadata?.city || metadata?.city_slug || "").trim();

    // Newer schema (beacon_system_complete): status=live, lat/lng, city
    ...
    if (city) patchNew.city = city;

    let up = await admin.from("beacons").update(patchNew).eq("id", beaconId);
    if (up.error) {
      // Legacy schema fallback (001_beacon_system.sql): status=active, geo_lat/geo_lng, city_slug
      const patchOld: Record<string, unknown> = {
        status: "active",
        starts_at: startsAt,
      };
      if (Number.isFinite(lat) && Number.isFinite(lng)) {
        patchOld.geo_lat = lat;
        patchOld.geo_lng = lng;
      }
      if (city) patchOld.city_slug = city.slice(0, 100);
      up = await admin.from("beacons").update(patchOld).eq("id", beaconId);
    }
  ...
  try {
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        // Always attempt side-effects idempotently (Stripe may retry / send multiple event types)
        ...
        // XP awarding (idempotent by checking xp_ledger)
        ...
        // Music purchases: spawn listener node + bonus XP (best-effort)
        await handleDigitalTrackPurchase(session?.metadata, order.buyer_id, orderId);

        // Optional: spawn/activate beacon on globe if checkout metadata included it
